...
async function seed() {
  ...
  let countryDK: CountryInstance,
    countryUS: CountryInstance,
    countryIT: CountryInstance;

  await step("Countries", async () => {
    countryDK = await CountryModel.createOne({
      id: 1,
      name: "Denmark",
      countryCode: "DK",
    });
    countryUS = await CountryModel.createOne({
      id: 2,
      name: "United States",
      countryCode: "US",
    });
    countryIT = await CountryModel.createOne({
      id: 3,
      name: "Italy",
      countryCode: "IT",
    });
  ...
  let brandNike: BrandInstance,
    brandZara: BrandInstance,
    brandGucci: BrandInstance;
  ...
  await step("Brand → Country relationships", async () => {
    ...
    await brandGucci.relateTo({
      alias: "country",
      where: { id: countryIT.id },
    });
  });
  ...
  let catShoes: CategoryInstance,
    catTops: CategoryInstance,
    catPants: CategoryInstance;
  ...
  let imgAirMax: ImageInstance,
    imgHoodie: ImageInstance,
    imgChinos: ImageInstance;

  await step("Images", async () => {
    imgAirMax = await ImageModel.createOne({
      id: 1,
      url: "https://cdn.example.com/items/air-max.jpg",
    });
    imgHoodie = await ImageModel.createOne({
      id: 2,
      url: "https://cdn.example.com/items/hoodie.jpg",
    });
    imgChinos = await ImageModel.createOne({
      id: 3,
      url: "https://cdn.example.com/items/chinos.jpg",
    });
  });
  ...
  let itemAirMax: ItemInstance,
    itemHoodie: ItemInstance,
    itemChinos: ItemInstance;

  await step("Items", async () => {
    itemAirMax = await ItemModel.createOne({
      id: 1,
      name: "Air Max 90",
      price: 129.99,
    });
    itemHoodie = await ItemModel.createOne({
      id: 2,
      name: "Classic Hoodie",
      price: 49.99,
    });
    itemChinos = await ItemModel.createOne({
      id: 3,
      name: "Slim Chinos",
      price: 69.99,
    });
  ...
  await step("Item relationships", async () => {
    ...
    await itemAirMax.relateTo({
      alias: "category",
      where: { id: catShoes.id },
    });
    ...
    await itemChinos.relateTo({
      alias: "category",
      where: { id: catPants.id },
    });
  ...
  await step("Closet → Item relationships", async () => {
    ...
    await closetSummer.relateTo({
      alias: "items",
      where: { id: itemAirMax.id },
    });
    await closetSummer.relateTo({
      alias: "items",
      where: { id: itemHoodie.id },
    });

    await closetFormal.relateTo({
      alias: "items",
      where: { id: itemChinos.id },
    });
    await closetFormal.relateTo({
      alias: "items",
      where: { id: itemHoodie.id },
  ...
  await step("Outfits", async () => {
    outfitCasual = await OutfitModel.createOne({
      id: 1,
      name: "Weekend Casual",
      style: "casual",
    });
    outfitSmart = await OutfitModel.createOne({
      id: 2,
      name: "Smart Friday",
      style: "smart-casual",
    });
  ...
  await step(
    "User → Outfit (CREATES) + Closet → Outfit (CREATES) relationships",
    async () => {
      const now = new Date().toISOString();
  ...
  await step("Reviews", async () => {
    reviewOne = await ReviewModel.createOne({
      id: 1,
      score: 5,
      text: "Love this combination — very versatile!",
    });
    reviewTwo = await ReviewModel.createOne({
      id: 2,
      score: 4,
      text: "Great for the office, very comfortable.",
    });
  });
  ...
  await step("Review relationships", async () => {
    ...
    // (Review)-[:ABOUT]->(Outfit)
    await reviewOne.relateTo({
      alias: "outfit",
      where: { id: outfitCasual.id },
    });
    await reviewTwo.relateTo({
      alias: "outfit",
      where: { id: outfitSmart.id },
    });
